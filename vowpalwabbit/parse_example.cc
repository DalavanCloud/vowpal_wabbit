...
/**
Compute feature index. If the feature is a numeric string then use is 
as a hash value. Else return hash value of the string content.

Also trim the string before hashing
...
/**
Compute hash of the feature name
@param s feature name
@param h seed value
*/
...
/**
Get hash function given the function name
*/
...
public:
  ...
  float cur_channel_v;
  bool  new_index;
  size_t mask; // hash bit mask - limit used bits in the hash value
  bool audit;
  size_t channel_hash; // feature namespace hash
  char* base; // feature namespace?
  size_t index;
  float v;
  parser* p;
  example* ae;
  ...
  */
  inline void maybeFeature(){
	  if(*reading_head == ' ' || *reading_head == '|'|| reading_head == endLine ){
		  ...
		  v = 1.0;
		  featureValue(); // read feature value
		  v *= cur_channel_v;
  ...
  */
  inline void nameSpaceInfo(){
	  if(reading_head == endLine ||*reading_head == '|' || *reading_head == ' ' || *reading_head == ':'){
		  ...
		  index = (unsigned char)(*reading_head); // namespace index is a first letter of the name?
  ...
  */
  ...
  inline void nameSpace(){
	  ...
	  if(*reading_head == ' ' || reading_head == endLine || *reading_head == '|'){
		  ...
		  index = (unsigned char)' '; // default index(namespace) is ' '
...
{
	all->p->lp->default_label(ae->ld); // init label data
