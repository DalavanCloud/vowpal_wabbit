	...
	float getLoss(shared_data* sd, float prediction, float label) {
		if (prediction <= sd->max_label && prediction >= sd->min_label)
		{
			...
			if (label == sd->min_label)
				...
				+ 2.0 * (label-sd->min_label) * (sd->min_label - prediction));
		} else {
			if (label == sd->max_label)
				return 0.0;
			else
				return float((sd->max_label - label) * (sd->max_label - label) 
				+ 2.0 * (sd->max_label - label) * (prediction - sd->max_label));
		}
	}
...
public:
	...
	*/
	float getUpdate(float prediction, float label, float eta_t, float norm) {
		...
		float err = (label * label - label * prediction)/(label * label); // relative err
		float normal = eta_t;
...
public:
	...
	inline float wexpmx(float x){
		...
		*/
		double w = x >= 1.0 ? 0.86*x + 0.01 : exp(0.8*x - 0.65); //initial guess
		double r = x >= 1. ? x - log(w) - w : 0.2 * x + 0.65 - w; //residual
		double t = 1.0 + w;
		double u = 2.0 * t * (t + 2.0* r /3.0); //magic
		return (float)(w * (1.0 + r / t*(u - r)/(u - 2.0 * r)) - x); //more magic
	}
